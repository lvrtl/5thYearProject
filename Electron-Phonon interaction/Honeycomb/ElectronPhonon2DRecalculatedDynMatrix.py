...
class lattice:
    ...
    @staticmethod
    def write(objList, filename):
        """
        takes a list of objects and saves it to a file
        all objects must be same length
        """
        data = [[objList[j][i] for j in range(len(objList))] 
                for i in range(len(objList[0]))]
...
#
    ...
    def NeighboursPlot(self,ax):
        ...
        out.append(plt.title("bonds in the lattice"))
            
    
if __name__ == "__main__":
    
    latticeDims = [5,5]
    lattice.HoneycombData(latticeDims,1,12)
    BC = None
    Grid = lattice('GridData.csv','MassIndex.csv',
                            'BondStretchingData.csv','BondLengthData.csv',
                            'BondBendingData.csv','IdealAngleData.csv',
                            BC)

    Grid.neighbourscalc(1)
    Grid.matcalc()
    Grid.EigSolve()
    ...
    if Grid.BoundaryConditions is None:
        Gridtype = 'GridFin'
    else:
        Gridtype = 'GridPeriodic'
    
    #create directory
    ImgDir = dir_path +  '\\' + str(latticeDims) + Gridtype + '\\'
    ...
    with open(mat_path, 'w',newline = '') as f:
        ...
        # write a row to the csv file
        writer.writerows(np.ndarray.tolist(Grid.DynamicalMatrix))


    
    '''
    Info for electron plotting
    '''
    Grid.sigma = 0.15
    r = [2,2,2]
    Grid.X0 = [(max([point.v[0] for point in Grid.points])
                +min([point.v[0] for point in Grid.points]))/2,
            (max([point.v[1] for point in Grid.points])
                +min([point.v[1] for point in Grid.points]))/2,0]
    ...
    fig = plt.figure()
    ax = mplot3d.axes3d.Axes3D(fig,zticks = [])
    ax.scatter3D([point.v[0] for point in Grid.points],
                    [point.v[1] for point in Grid.points],[0 for point in Grid.points])
    for points in Grid.points:
            for neighbour in points.neighbours:
                ax.plot3D([points.v[0],neighbour.v[0]],
                    [points.v[1],neighbour.v[1]],[0,0],color = 'red')
    plt.title("5 by 5 honeycomb structure")
    graph_path_phonon = ImgDir+'\\'+'picture.svg'
...
#    Grid.R0 = r
#    Grid.eOverlap()
#    Grid.gaussianRepresentation(200)
#    
#    fig, ax = plt.subplots(1,1)
#    Grid.NeighboursPlot(ax)
...
#    
#    
#    paramsFitting = chain.RadiusFitting(r)
#    plt.scatter([x[0] for x in r], paramsFitting)
#    plt.xlabel("electron radius")
#    plt.ylabel("modelled radius")
#    plt.title("theoretical model scale fitting over radius")
#    plt.xlim([0, np.max(r)+1])
#    plt.ylim([0,np.max(paramsFitting)+1])
#    
#    
#    def LinFit(params):
#        return params[0] + params[1] * np.array([x[0] for x in r])
#    def FittingFunc(params):
#        return LinFit(params) - paramsFitting
#    paramsStraightLine = leastsq(FittingFunc, [0,1])
#    plt.plot([0,np.max(r)],[paramsStraightLine[0][0],
#              paramsStraightLine[0][0] + np.max(r) * paramsStraightLine[0][1]])
...
#    
    ...
    """
    plot of Gaussian weighted electron phonon interaction
    """
#    fig, ax = plt.subplots(1,1)
#    Grid.electronPhononInteractionGaussian(ax)
...
#    graph_path_GkFitting = ImgDir+'\\'+'GkFitting.svg'
#    if os.path.exists(graph_path_GkFitting) == False:
#        open(graph_path_GkFitting,'x')
#    plt.savefig(graph_path_GkFitting) 
